export const en = {
  ui: {
    gold: "Gold",
    gps: "Per second",
    click: "Manual Mine",
    buy: "Buy",
    menu: "Menu",
    main: "Main",
    upgrades: "Shop",
    stats: "Stats",
    prestige: "Prestige",
    reset: "Reset Save",
    continue: "Continue",
    new_game: "New Game",
    achievements: "Achievements",
    offline_msg: "You were away for {{time}} min. Offline earnings: {{earned}} 💰",
    click_label: "Manual Mine (+{{power}})",
    gold_plus: "Gold +{{power}}",
    buy_cost: "Buy: {{cost}} 💰",
    upgrade_buy_btn: "Buy ({{cost}})",
    lang_name: "English",
    log_title: "Journal",
    save_manager: 'Saves',
    cancel: 'Cancel',
  },
  logs: {
    system_active: "Systems active...",
    buy_building: "Purchased: {{name}}",
    buy_upgrade: "Unlocked: {{name}}",
    prestige_low: "Not enough gold for prestige!",
    era_started: "ERA {{n}} STARTED!",
    prestige_multiplier: "Prestige multiplier: x{{val}}",
    save_imported: 'Save imported successfully!',
    import_failed: 'Import failed: {{error}}',
    backup_restored: 'Backup restored!',
    save_corrupted: 'Save is corrupted or tampered!',
    export_ready: 'Save ready for download!',
  },
  prestige: {
    title: "Alchemy Laboratory",
    points: "Prestige Points",
    bonus: "Current bonus",
    possible_gain: "Possible gain",
    pts: "pts",
    requirement: "(Minimum 1M gold required to earn points)",
    execute: "Execute Prestige (Reset)",
    confirm: "Are you sure you want to Prestige?\n\nYou will lose all buildings and gold, but receive +{{gain}} Prestige points.\nYour production will increase by another {{percent}}%!",
  },
  stats: {
    title: "Empire Statistics",
    gold: "Gold",
    speed: "Speed",
    click: "Click",
    cycles_title: "Production Cycles",
    cycles_empty: "Buy a building to see cycle details.",
    cycles_building: "Building",
    cycles_time: "Cycle Time",
    cycles_progress: "Progress",
    cycles_payout: "Payout / cycle",
    cycles_global_modifier: "Global cycle modifier",
    back: "Back",
  },
  upgrades_shop: {
    title: "Upgrade Shop",
  },
  achievements_page: {
    title: "🏆 Achievements",
    unlocked: "✓ Unlocked",
    production_bonus: "+{{pct}}% to production",
  },
  upgrades: {
    sharp_pickaxe: { name: "Sharp Pickaxes", desc: "Manual x2" },
    miner_motivation: { name: "Motivation", desc: "Miners x2" },
    cosmic_energy: { name: "Cosmic Energy", desc: "Global x2" },
    miner_rhythm_training: { name: "Rhythm Training", desc: "Miner cycle -200 ms" },
    drill_cooling: { name: "Active Cooling", desc: "Drill cycle x0.8" },
    factory_shift_optimization: { name: "Shift Optimization", desc: "Factory x1.25, cycle x0.7" },
    temporal_coordination: { name: "Temporal Coordination", desc: "Global cycle x0.9" },
    nano_dispatch: { name: "Nano Dispatch", desc: "Global cycle -50 ms" },
  },
  achievements: {
    firstGold: { name: "First Steps", desc: "Earn 100 gold" },
    minerCollector: { name: "Miner Collector", desc: "Own 5 miners" },
    moneyMaker: { name: "Money Maker", desc: "Earn 10000 gold" },
    speedDemon: { name: "Speed Demon", desc: "Reach 50 GPS" },
  },
  buildings: {
    miner: { name: "Miner", desc: "Produces 1/s" },
    drill: { name: "Drill", desc: "Generates 5/s" },
    factory: { name: "Factory", desc: "Produces 25/s" },
  },
  save_manager: {
    title: 'Save Management',
    export_section: 'Export Save',
    import_section: 'Import Save',
    download_file: 'Download File',
    backups_section: 'Backups',
    restore_button: 'Restore',
    dev_mode_warning: '⚠️ DEV MODE ACTIVE - Saves are NOT encrypted!',
    no_backups: 'No backups available',
    confirm_restore: 'Are you sure you want to restore from {{date}}? Current game state will be replaced.',
    backup_restored_title: 'Backup restored!',
    import_success: 'Save loaded successfully!',
    confirm_restore_title: 'Restore Backup?',
    backup_restored_message: 'Game state from {{date}} has been loaded.',
    restore_failed: 'Restore Failed',
    import_success_title: 'Save Loaded!',
    import_failed_title: 'Import Failed',
  },
};
